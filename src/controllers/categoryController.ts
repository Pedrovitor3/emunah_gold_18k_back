...
export const getCategories = async (
  ...
  try {
    ...
    if (search) {
      queryBuilder.andWhere('(c.name ILIKE :search OR c.description ILIKE :search)', { 
        search: `%${search}%` 
      });
    }
...
export const getActiveCategories = async (_request: FastifyRequest, reply: FastifyReply) => {
  try {
    const categoryRepo = AppDataSource.getRepository(Category);
